...
import { formatVoiceBootstrapMemberSummary } from "@/lib/voiceBootstrapLog";
...
import {
  ...
  isPeerUiConnectedHoldActive,
  shouldAllowConnectedRecovery,
  type PeerConnectedHoldRecord,
} from "@/lib/voiceConnectedHold";
import {
  logVoiceMuteApply,
  logVoiceMuteRenegotiateNeeded,
  logVoiceMuteRenegotiateSkipped,
  MUTE_TOGGLE_DEBOUNCE_MS,
...
function isRejoinNegotiationReason(reason: string): boolean {
  return (
    ...
    reason.startsWith("soft_reset_") ||
    reason === "member_rejoin"
  );
}

function resolveBlockingPipelineMark(
  marks: Record<string, boolean>
): string | null {
  if (marks.audio_confirmed_strict) return "audio_confirmed_strict";
  if (marks.offer_received) return "offer_received";
  if (marks.answer_received) return "answer_received";
  if (marks.remote_track_received) return "remote_track_received";
  if (marks.offer_sent) return "offer_sent";
  if (marks.answer_sent) return "answer_sent";
  return null;
}
...
function findPeerAudioSender(pc: RTCPeerConnection): RTCRtpSender | null {
  ...
  const liveAudio = senders.find((s) => s.track?.kind === "audio");
  if (liveAudio) return liveAudio;
  const nullTrackSender = senders.find((s) => s.track == null);
  if (nullTrackSender) return nullTrackSender;
...
function peerHasLiveLocalAudioSender(pc: RTCPeerConnection): boolean {
  return pc
    .getSenders()
    .some(
      (sender) =>
...
export function usePeerConnections({
  ...
  useEffect(() => {
    ...
    softResetPerConnectionRef.current.clear();
    ...
    remoteVoiceEpochTracksRef.current.clear();
    remoteJoinPhaseRef.current.clear();
    localReentryResetSentRef.current.clear();
  ...
  const remoteVoiceEpochTracksRef = useRef<Map<string, RemoteVoiceEpochTrack>>(
    new Map()
  );
  const remoteJoinPhaseRef = useRef<Map<string, RemoteJoinPhase>>(new Map());
  const localCallEntryGenerationRef = useRef(
    bumpLocalCallEntryGeneration(sessionId)
  );
  const localReentryResetSentRef = useRef<Set<string>>(new Set());
  const softResetAttemptCountRef = useRef<Map<string, number>>(new Map());
  const softResetLastAtRef = useRef<Map<string, number>>(new Map());
  const softResetPerConnectionRef = useRef<Map<string, string>>(new Map());
  ...
  const pollPeerAudioDiagnosticsRef = useRef<
    (remoteId: string, opts?: { forceLog?: boolean }) => void
  >(() => {});
  const ensurePeerLocalAudioSenderRef = useRef<
    (remoteId: string, reason: string) => Promise<boolean>
  >(async () => false);
  const attachLocalAudioToAllPeersRef = useRef<
  ...
  const muteDebounceTimerRef = useRef<number | null>(null);
  const unmuteOutboundRepairTimersRef = useRef<Map<string, number>>(new Map());
  ...
  const clearPeerNegotiationCycleState = useCallback(
    (remoteId: string, reason: string) => {
      ...
      bidirectionalEstablishedRef.current.delete(remoteId);
      ...
      peerSignalTimestampsRef.current.set(
        remoteId,
        emptyPeerSignalTimestamps()
      );
      resetPeerAudioDiagnostics(remoteId);
      debugConsoleLog(
        `[voice-peer] negotiation-cycle-reset remote=${compactDeviceId(remoteId)} reason=${reason}`
      );
    },
    [clearPassiveWaitOfferTimer]
  );

  const setRemoteJoinPhase = useCallback(
    (remoteId: string, phase: RemoteJoinPhase) => {
      remoteJoinPhaseRef.current.set(remoteId, phase);
    },
    []
  );
  ...
  const attemptPeerAudioStrictRecovery = useCallback(
    async (
      ...
      if (subClass === "D5") {
        await ensurePeerLocalAudioSenderRef.current(remoteId, "audio_strict_d5");
  ...
  const pollPeerAudioDiagnostics = useCallback(
    async (remoteId: string, opts?: { forceLog?: boolean }) => {
      const pc = pcsRef.current.get(remoteId);
  ...
  pollPeerAudioDiagnosticsRef.current = (remoteId, opts) => {
    void pollPeerAudioDiagnostics(remoteId, opts);
  };
  ...
  const handleRemotePlaybackHealthChange = useCallback(
    (remoteId: string, health: RemotePlaybackHealth) => {
      ...
      } else if (health.playSuccess && !health.audioConfirmedStrict) {
        markVoicePerf("audio_provisional", { remoteId });
      }

      if (health.audioConfirmedStrict) {
        bidirectionalEstablishedRef.current.add(remoteId);
        setRemoteJoinPhase(remoteId, "established");
        touchPeerSignal(remoteId, "playback_confirmed");
        markVoicePerf("audio_confirmed_strict", { remoteId });
        markVoicePerf("audio_confirmed", { remoteId });
      ...
      hasStrongInboundPlaybackEvidence({
        ...
        voiceProdLog(
          `[voice-peer] playback_evidence remote=${compactDeviceId(remoteId)} ` +
            `playbackActive=${health.playbackActive ? 1 : 0} ` +
            `currentTimeAdvanced=${health.currentTimeAdvanced ? 1 : 0} ` +
  ...
  const shouldRejectIncomingStaleOffer = useCallback(
    (
      ...
      const reject = shouldRejectEstablishedPeerStaleOffer({
        currentConnectionId: getCurrentConnectionId(remoteId),
      ...
      if (!reject && inboundRtpMissing) {
        voiceProdLog(
          `[voice-signal] stale-offer-accept-one-way remote=${compactDeviceId(remoteId)} ` +
            `incoming=${compactConnectionId(incomingConnectionId)} ` +
            `current=${compactConnectionId(getCurrentConnectionId(remoteId))} ` +
            `inboundDeltaBytes=${inboundDeltaBytes} inboundDeltaPkts=${inboundDeltaPackets}`
        );
      }
  ...
  const closePeer = useCallback(
    (
      ...
      const unmuteRepairTimer = unmuteOutboundRepairTimersRef.current.get(remoteId);
      if (unmuteRepairTimer != null) {
        window.clearTimeout(unmuteRepairTimer);
        unmuteOutboundRepairTimersRef.current.delete(remoteId);
      }
  ...
  const createPeerConnection = useCallback(
    (remoteId: string, connectionId: string, opts?: VoicePeerCreateOpts) => {
      ...
      if (existing && currentId === connectionId) {
        queueMicrotask(() => {
          void ensurePeerLocalAudioSenderRef.current(remoteId, "create_pc_reuse");
        });
        return existing;
      }
      ...
      clearPeerNegotiationCycleState(remoteId, `create_pc:${mutationReason}`);
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (state === "connected") {
          ...
          if (track && userMutedRef.current) {
            track.enabled = false;
          } else if (sender && track && !peerHasLiveLocalAudioSender(pc)) {
  ...
  const startPeerOffer = useCallback(
    async (
      ...
      if (!canSendVoiceOffer() && isOfferOwner) {
        voiceProdLogOnStateChange(
          `voice-bootstrap:offer-blocked:${remoteId}`,
          `mic|${getLocalTrackReadyState(localAudioTrackRef, localStreamRef)}|${reason}`,
          `[voice-bootstrap] action=skip remote=${compactDeviceId(remoteId)} ` +
            `skipReason=local_track_not_live hasUsablePc=${isUsablePeerConnection(pcsRef.current.get(remoteId)) ? 1 : 0} ` +
            `isOfferOwner=1 canSendVoiceOffer=0 ` +
      ...
      await ensurePeerLocalAudioSenderRef.current(remoteId, reason);
  ...
  >(() => {});
  const sendReconnectRequestRef = useRef<
    (
      remoteId: string,
      connectionId: string,
      reason: string,
    ...
    (remoteId: string, connectionId: string, reason: string) => {
      ...
      const timer = window.setTimeout(() => {
        ...
        if (answerWaitRetriedByConnRef.current.get(remoteId) === connectionId) {
          voiceProdLog(
            `[voice-signal] answer_wait_recovery remote=${compactDeviceId(remoteId)} ` +
              `action=reconnect_and_reschedule connectionId=${compactConnectionId(connectionId)} ` +
              `currentConnectionId=${compactConnectionId(currentConnectionId)} elapsedMs=${ageMs}`
          );
          peerAutoRecoveryFrozenRef.current.delete(remoteId);
          offeredPeersRef.current.delete(remoteId);
          void sendReconnectRequestRef.current(
            remoteId,
            connectionId,
            "answer_wait_timeout"
          );
        ...
        voiceProdLog(
          `[voice-signal] answer_wait_recovery remote=${compactDeviceId(remoteId)} ` +
            `action=reconnect_request_and_retry_offer connectionId=${compactConnectionId(connectionId)} ` +
            `currentConnectionId=${compactConnectionId(currentConnectionId)} elapsedMs=${ageMs} ` +
            `sig=${pc.signalingState}`
        );
        void sendReconnectRequestRef.current(
  ...
  const renegotiatePeerForLocalAudio = useCallback(
    async (remoteId: string, reason: string): Promise<boolean> => {
      const pc = pcsRef.current.get(remoteId);
      if (!pc || !isUsablePeerConnection(pc)) return false;
      if (
        pc.signalingState !== "stable" &&
      ...
      const now = Date.now();
      const lastAt = localAudioRenegotiateAtRef.current.get(remoteId) ?? 0;
      if (now - lastAt < 3000) return false;

      try {
        voiceProdLog(
          `[voice-peer] local-audio-renegotiate-start remote=${compactDeviceId(remoteId)} ` +
            `reason=${reason} owner=${deviceId < remoteId ? 1 : 0}`
        );
        const offer = await pc.createOffer({ offerToReceiveAudio: true });
        if (pcsRef.current.get(remoteId) !== pc || pc.signalingState !== "stable") {
          return false;
        }
        ...
        if (!sendResult.ok) {
          voiceProdLog(
            `[voice-peer] local-audio-renegotiate-failed remote=${compactDeviceId(remoteId)} ` +
              `reason=${reason} name=${sendResult.errorName ?? "unknown"}`
          );
          return false;
        }
        ...
        markVoiceNegotiationStep(
          remoteId,
          "offer_send",
          `reason=${reason}_local_audio_renegotiate`
        );
        touchPeerSignal(remoteId, "offer_sent");
        localAudioRenegotiateAtRef.current.set(remoteId, now);
        voiceProdLog(
          `[voice-peer] local-audio-renegotiate-offer-sent remote=${compactDeviceId(remoteId)} reason=${reason}`
        );
        ...
        console.warn("[voice-peer] local-audio-renegotiate failed", remoteId, e);
        return false;
      }
    },
    [
  ...
  const ensurePeerLocalAudioSender = useCallback(
    async (
      remoteId: string,
      reason: string,
      opts?: { allowRenegotiate?: boolean }
    ): Promise<boolean> => {
      const pc = pcsRef.current.get(remoteId);
      if (!pc || !isUsablePeerConnection(pc)) return false;

      if (
        isReceiveOnlyMutedSession(voicePolicy.releaseMicOnMute, userMutedRef)
      ) {
        return false;
      }
      ...
      if (!track || track.readyState !== "live" || !stream) {
        voiceProdLog(
          `[voice-peer] local-audio-sender-skip remote=${compactDeviceId(remoteId)} ` +
            `reason=${reason} trackReady=${track?.readyState ?? "none"} hasStream=${stream ? 1 : 0}`
        );
        return false;
      }
      ...
      if (!track.enabled) {
        track.enabled = true;
        logVoiceMuteApply({ enabled: true, reason: `${reason}_ensure_sender` });
      }

      if (peerHasLiveLocalAudioSender(pc)) {
        const liveSender = findPeerAudioSender(pc);
        if (liveSender?.track?.id === track.id) {
          logVoiceMuteRenegotiateSkipped({
            reason: "sender_live",
            remoteId,
          });
      ...
      let sender = findPeerAudioSender(pc);
      let needsRenegotiation = false;

      if (!sender) {
        pc.addTrack(track, stream);
        sender = findPeerAudioSender(pc);
        ...
        voiceProdLog(
          `[voice-peer] local-audio-add-track remote=${compactDeviceId(remoteId)} ` +
            `reason=${reason} trackId=${track.id.slice(-8)}`
        );
      } else if (sender.track?.readyState === "ended") {
        await sender.replaceTrack(track);
        logVoicePeerReplaceTrack(remoteId, track, reason, { prod: true });
        needsRenegotiation = true;
        logVoiceMuteRenegotiateNeeded({
          reason: "sender_track_dead",
          remoteId,
        });
      ...
      if (!sender) {
        voiceProdLog(
          `[voice-peer] local-audio-sender-missing remote=${compactDeviceId(remoteId)} reason=${reason}`
        );
        return false;
      }
      ...
      if (shouldRenegotiate) {
        await renegotiatePeerForLocalAudio(
          remoteId,
          `${reason}_sender_attach`
        );
      } else if (needsRenegotiation) {
        logVoiceMuteRenegotiateSkipped({
          reason: "renegotiate_deferred",
          remoteId,
        });
      }

      return peerHasLiveLocalAudioSender(pc);
    },
    [
      localAudioTrackRef,
      localStreamRef,
      renegotiatePeerForLocalAudio,
      signalReady,
      userMutedRef,
      voicePolicy.releaseMicOnMute,
    ]
  ...
  ensurePeerLocalAudioSenderRef.current = ensurePeerLocalAudioSender;

  const clearUnmuteOutboundRepairTimer = useCallback((remoteId: string) => {
    const timer = unmuteOutboundRepairTimersRef.current.get(remoteId);
    if (timer != null) {
      window.clearTimeout(timer);
      unmuteOutboundRepairTimersRef.current.delete(remoteId);
    }
  ...
  const scheduleUnmuteOutboundRepair = useCallback(
    (remoteId: string, reason: string) => {
      clearUnmuteOutboundRepairTimer(remoteId);
      const timer = window.setTimeout(() => {
        unmuteOutboundRepairTimersRef.current.delete(remoteId);
        ...
        if (peerHasLiveLocalAudioSender(pc)) {
          logVoiceMuteRenegotiateSkipped({
            reason: "outbound_repair_sender_live",
            remoteId,
          });
          return;
        }
        if (getPeerOutboundDeltaBytes(remoteId) > 0) {
          logVoiceMuteRenegotiateSkipped({
            reason: "outbound_repair_delta_ok",
            remoteId,
          });
          return;
        }
        logVoiceMuteRenegotiateNeeded({
          reason: "outbound_still_zero",
          remoteId,
        });
        void ensurePeerLocalAudioSender(remoteId, `${reason}_outbound_repair`, {
          allowRenegotiate: true,
        });
      }, UNMUTE_OUTBOUND_REPAIR_DELAY_MS);
      unmuteOutboundRepairTimersRef.current.set(remoteId, timer);
    },
    [clearUnmuteOutboundRepairTimer, ensurePeerLocalAudioSender]
  );

  const attachLocalAudioToAllPeers = useCallback(
    async (reason: string): Promise<number> => {
      if (
        ...
        voiceProdLog(
          `[voice-peer] local-audio-attach-deferred reason=${reason} ` +
            `trackReady=${getLocalTrackReadyState(localAudioTrackRef, localStreamRef)}`
        );
        return 0;
      }
      ...
      for (const remoteId of remoteIds) {
        const pc = pcsRef.current.get(remoteId);
        if (!pc || !isUsablePeerConnection(pc)) continue;

        const track = localAudioTrackRef.current;
        if (track && peerHasLiveLocalAudioSender(pc)) {
          const sender = findPeerAudioSender(pc);
        ...
        const hold = getPeerConnectedHoldRecord(remoteId);
        const onConnectedHold = isPeerUiConnectedHoldActive(hold, now);
        const ok = await ensurePeerLocalAudioSender(remoteId, reason, {
          allowRenegotiate: !onConnectedHold,
        });
        if (ok) {
          attached += 1;
          clearUnmuteOutboundRepairTimer(remoteId);
        ...
        if (onConnectedHold) {
          scheduleUnmuteOutboundRepair(remoteId, reason);
          continue;
        }
      }
      ...
      if (attached > 0) {
        voiceProdLog(
          `[voice-peer] local-audio-attach-batch reason=${reason} peers=${attached}`
        );
      }
    ...
    [
      clearUnmuteOutboundRepairTimer,
      ensurePeerLocalAudioSender,
      getPeerConnectedHoldRecord,
      getRemoteIds,
      localAudioTrackRef,
      localStreamRef,
      scheduleUnmuteOutboundRepair,
      userMutedRef,
      voicePolicy.releaseMicOnMute,
    ]
  );

  attachLocalAudioToAllPeersRef.current = attachLocalAudioToAllPeers;

  const runPassiveFallbackOffer = useCallback(
    (remoteId: string, triggerReason: string) => {
      const joinPhase = remoteJoinPhaseRef.current.get(remoteId);
      if (!allowsPassiveFallbackOffer(joinPhase)) {
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
          reason: "not_initial_connect",
          detail: joinPhase ?? "unset",
        });
        return;
      }
      ...
      if (shouldSuppressAutoVoiceRecoveryForPeer(remoteId)) {
        logPassiveOfferSkip({
          ...
          detail: "fallback_offer",
        });
        return;
      }

      const marks = getPeerPipelineMarks(remoteId);
      const blockingMark = resolveBlockingPipelineMark(marks);
      if (blockingMark) {
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
          reason: "pipeline_mark",
          detail: blockingMark,
        });
        return;
      }
      ...
      if (passiveFallbackOfferByConnRef.current.get(remoteId) === connectionId) {
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
          reason: "fallback_already_sent",
          detail: compactConnectionId(connectionId),
        });
        return;
      }
      ...
      if (
        ...
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
          reason: "signaling_busy",
          detail: `${pc.signalingState}/${pc.connectionState}`,
        });
        return;
      }
  ...
  const schedulePassiveWaitOfferTimeout = useCallback(
    (
      ...
      const isPassiveRole = deviceId > remoteId;
      if (!isPassiveRole) {
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
      ...
      if (isPassiveAwaitingReconnectOffer(deviceId, remoteId, passiveReconnectStateRef)) {
        ...
        logPassiveOfferSkip({
          ...
          triggerReason,
          reason: "awaiting_reconnect_offer",
          detail: awaitState?.reconnectReason ?? "passive_wait",
        });
        return false;
      }

      const joinPhase = remoteJoinPhaseRef.current.get(remoteId);
      if (!allowsPassiveFallbackOffer(joinPhase, opts)) {
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
          reason: "not_initial_connect",
      ...
      if (shouldSuppressAutoVoiceRecoveryForPeer(remoteId)) {
        logPassiveOfferSkip({
          ...
          triggerReason,
          reason: "auto_recovery_frozen",
          detail: getEstablishedPeerSkipReasonForPeer(remoteId) ?? "unknown",
        });
        return false;
      }
      ...
      if (!canSendVoiceOffer() && !isPassiveRole) {
        logPassiveOfferSkip({
          remoteId,
          triggerReason,
          reason: "mic_blocked",
          detail: getMicOfferBlockReason(),
        });
        return false;
      }
  ...
  const beginPassiveOfferWait = useCallback(
    (
      ...
      passiveJoinSettledRef.current.delete(remoteId);
      if (isLocalTrackLive(localAudioTrackRef, localStreamRef)) {
        void ensurePeerLocalAudioSenderRef.current(remoteId, triggerReason);
      }
  ...
  const sendReconnectRequest = useCallback(
    async (
      ...
      const result = await sendSignal(remoteId, "reconnect-request", {
        ...
        resetReason: reason,
        voiceEpoch:
          remoteVoiceEpochTracksRef.current.get(remoteId)?.epoch ??
          localCallEntryGenerationRef.current,
      });
  ...
  useEffect(() => {
    sendReconnectRequestRef.current = sendReconnectRequest;
  ...
  const resetRemotePeerForRejoin = useCallback(
    async (
      remoteId: string,
      reason: "remote_rejoin" | "local_rejoin" | "reconnect_request_received",
      voiceEpoch: number,
      opts?: {
        incomingConnectionId?: string;
        offerReason?: string;
      ...
      peerAutoRecoveryFrozenRef.current.delete(remoteId);
      bidirectionalEstablishedRef.current.delete(remoteId);
      manualHardResetHealPassRef.current.delete(remoteId);
      answerWaitRetriedByConnRef.current.delete(remoteId);
      softResetAttemptCountRef.current.delete(remoteId);
      softResetLastAtRef.current.delete(remoteId);
      softResetPerConnectionRef.current.delete(remoteId);
      softResetExhaustedNotifiedRef.current.delete(remoteId);
      ensurePeerAttemptRef.current.delete(remoteId);
      activeOfferJoinLoggedRef.current.delete(remoteId);

      clearPeerNegotiationCycleState(remoteId, reason);
      clearPassiveReconnectState(remoteId);
      clearAnswerWaitTimer(remoteId, reason, "resetRemotePeerForRejoin");
      clearPeerWatchdogTimers(remoteId);
      clearReconnectTimer(remoteId);
      reconnectPendingRef.current.delete(remoteId);
      audioStrictRecoveryAttemptedRef.current.delete(remoteId);
      ...
      const newConnectionId =
        opts?.incomingConnectionId ?? makeConnectionId(deviceId, remoteId);
      ...
      if (isOfferOwner) {
        ...
        if (!opts?.skipReconnectRequest && reason !== "reconnect_request_received") {
          await sendReconnectRequest(remoteId, newConnectionId, reason);
        }
      ...
      voiceProdLog(
        `[voice-peer] waiting_for_active_offer remote=${compact} reason=${reason} ` +
          `epoch=${voiceEpoch} connectionId=${compactConnectionId(newConnectionId)}`
      );
      ...
      passiveReconnectStateRef.current.set(remoteId, {
        ...
        sentAt:
          !opts?.skipReconnectRequest && reason !== "reconnect_request_received"
            ? Date.now()
            : null,
        retryUsed: false,
        retryTimerId: null,
      ...
      createPeerConnection(remoteId, newConnectionId, {
        caller: "resetRemotePeerForRejoin",
        reason: offerReason,
        force: true,
      });
      setPeerState(remoteId, "connecting");
  ...
  const ensurePeerConnection = useCallback(
    (
      ...
      if (hasUsablePc && !force) {
        ...
        if (
          existing &&
          !isReceiveOnlyMutedSession(
            voicePolicy.releaseMicOnMute,
            userMutedRef
          ) &&
          isLocalTrackLive(localAudioTrackRef, localStreamRef) &&
          !peerHasLiveLocalAudioSender(existing) &&
          isTransportMediaConnected(
            existing.connectionState,
            existing.iceConnectionState
          )
        ) {
          void ensurePeerLocalAudioSenderRef.current(
            remoteId,
            `${reason}_connected_missing_sender`
          );
        }
        if (shouldSuppressPassiveOfferReschedule(phase)) {
          passiveJoinSettledRef.current.add(remoteId);
        ...
        if (phase === "idle_unnegotiated") {
          if (isOfferOwner) {
            void ensurePeerLocalAudioSenderRef.current(remoteId, reason);
            if (canSendVoiceOffer()) {
  ...
  const runPeerSoftReset = useCallback(
    async (remoteId: string, triggerReason: VoiceSoftResetTriggerReason) => {
      ...
      if (currentConnectionId) {
        softResetPerConnectionRef.current.set(remoteId, currentConnectionId);
      }
      ...
      debugConsoleLog(
        `[voice-soft-reset] start remote=${compactDeviceId(remoteId)} ` +
          ...
          `reason=${triggerReason} oldConnectionId=${compactConnectionId(currentConnectionId)} ` +
          `${formatVoiceModeSuffix()}`
      );
      ...
      reconnectPendingRef.current.delete(remoteId);
      clearPassiveReconnectState(remoteId);
      clearPeerNegotiationCycleState(remoteId, `soft_reset_${triggerReason}`);
      pendingIceRef.current.delete(remoteId);
      audioStrictRecoveryAttemptedRef.current.delete(remoteId);
      oneWayAudioLoggedRef.current.forEach((key) => {
        if (key === remoteId || key.startsWith(`${remoteId}:`)) {
      ...
      const reconnectReason = `soft_reset_${triggerReason}`;
      voiceProdLog(
        `[voice-soft-reset] reconnect-request remote=${compactDeviceId(remoteId)} ` +
          `reason=${triggerReason} oldConnectionId=${compactConnectionId(currentConnectionId)} ` +
          `newConnectionId=${compactConnectionId(newConnectionId)} role=${isOfferOwner ? "active" : "passive"}`
      );
      ...
      if (isOfferOwner) {
        ...
        voiceProdLog(
          `[voice-peer] waiting_for_active_offer remote=${compactDeviceId(remoteId)} ` +
            `reason=${reconnectReason} connectionId=${compactConnectionId(newConnectionId)} ` +
      ...
      debugConsoleLog(
        `[voice-soft-reset] done remote=${compactDeviceId(remoteId)} ` +
          ...
          `reconnectSent=${isOfferOwner ? 1 : passiveReconnectStateRef.current.get(remoteId)?.sentAt != null ? 1 : 0} ` +
          `${formatVoiceModeSuffix()}`
      );
    },
    [
  ...
  const evaluateAndRunVoiceSoftResetForPeer = useCallback(
    async (remoteId: string) => {
      ...
      const negotiationComplete =
        ...
        marks.remote_track_received &&
        (marks.offer_sent || passiveFallbackOfferSent);
      const userIntentionallyMuted = isReceiveOnlyMutedSession(
        voicePolicy.releaseMicOnMute,
        userMutedRef
      );
      const softResetAlreadyOnConnection =
        currentConnectionId != null &&
        softResetPerConnectionRef.current.get(remoteId) === currentConnectionId;
      ...
      const trigger = evaluateVoiceSoftResetTrigger({
        ...
        userIntentionallyMuted,
        negotiationComplete,
        passiveFallbackOfferSent,
        softResetAlreadyOnConnection,
        awaitingActiveOffer:
          remoteJoinPhaseRef.current.get(remoteId) === "awaiting_active_offer",
      });
    ...
    for (const remoteId of remoteIds) {
      ...
      if (hasStream && connected) {
        if (
          ...
          !receiveOnly &&
          localTrackLive &&
          !peerHasLiveLocalAudioSender(pc)
        ) {
          void ensurePeerLocalAudioSenderRef.current(
            remoteId,
            "heal_connected_missing_sender"
          );
        }
        setPeerState(remoteId, "connected");
        continue;
    ...
    async (row: SignalRow) => {
      ...
      if (row.signal_type === "reconnect-request") {
        ...
        const offerReason =
          ...
          requestReason === "no_offer_after_auto_hard_reset"
            ? "auto_hard_reset"
            : requestReason.startsWith("soft_reset") ||
                requestReason === "answer_wait_timeout"
              ? "soft_reset_rejoin"
              : requestReason === "remote_rejoin" ||
                  requestReason === "local_rejoin"
                ? requestReason
                : "manual_reconnect";
        const incomingVoiceEpoch = Number(payload.voiceEpoch ?? 0);

        voiceProdLog(
          `[voice-signal] reconnect-request-received remote=${compactDeviceId(remoteId)} ` +
            `resetReason=${requestReason} offerReason=${offerReason} ` +
        ...
        if (incomingVoiceEpoch > 0) {
          const track =
            remoteVoiceEpochTracksRef.current.get(remoteId) ??
            createEmptyRemoteVoiceEpochTrack();
          if (incomingVoiceEpoch > track.epoch) {
            remoteVoiceEpochTracksRef.current.set(remoteId, {
              ...track,
              epoch: incomingVoiceEpoch,
              initialized: true,
              lastInCall: true,
            });
          }
        ...
        voiceProdLog(
          `[voice-signal] reconnect-request-handling remote=${compactDeviceId(remoteId)} ` +
            `role=${isOfferOwner ? "active" : "passive"} ` +
            `decision=${isOfferOwner ? "offer-send" : "wait_for_offer"} ` +
            `connectionId=${compactConnectionId(incomingConnectionId)} offerReason=${offerReason}`
        );

        await resetRemotePeerForRejoin(
          remoteId,
          "reconnect_request_received",
          incomingVoiceEpoch > 0
            ? incomingVoiceEpoch
            : (remoteVoiceEpochTracksRef.current.get(remoteId)?.epoch ?? 1),
          {
            incomingConnectionId,
            offerReason,
      ...
      try {
        if (row.signal_type === "offer") {
          ...
          await ensurePeerLocalAudioSenderRef.current(
            remoteId,
            "inbound_offer_answer"
          );
  ...
  const applyLocalAudioTrack = useCallback(
    (track: MediaStreamTrack | null, reason: string) => {
      ...
      if (!userMutedRef.current && !track.enabled) {
        track.enabled = true;
        voiceProdLog(
          `[voice-peer] local-track-enabled remote=- reason=${reason}_apply_local_track`
        );
      }

      void attachLocalAudioToAllPeers(reason).then((attached) => {
        if (attached > 0 && micReady && signalReady) {
          healPeerConnections();
        }
      });
    },
  ...
  const applyUserMutedTrackChange = useCallback(
    (nextMuted: boolean) => {
      ...
      if (track.enabled !== prevEnabled) {
        logVoiceMuteApply({ enabled: track.enabled });
      }
    },
    [localAudioTrackRef, userMutedRef]
  );

  const applyDebouncedUserMutedPeerAttach = useCallback(
    (nextMuted: boolean) => {
      if (nextMuted) {
        return;
      }
      ...
      let allSendersLive = remoteIds.size > 0;
      for (const remoteId of remoteIds) {
        const pc = pcsRef.current.get(remoteId);
        if (!pc || !isUsablePeerConnection(pc)) {
          allSendersLive = false;
          continue;
        }
        if (!peerHasLiveLocalAudioSender(pc)) {
          allSendersLive = false;
      ...
      if (allSendersLive && remoteIds.size > 0) {
        logVoiceMuteRenegotiateSkipped({ reason: "all_senders_live" });
        const now = Date.now();
        for (const remoteId of remoteIds) {
          const pc = pcsRef.current.get(remoteId);
          const hold = getPeerConnectedHoldRecord(remoteId);
          if (
            pc?.connectionState === "connected" ||
            hold.softAtMs != null ||
            hold.strictAtMs != null ||
            isPeerUiConnectedHoldActive(hold, now)
          ) {
            scheduleUnmuteOutboundRepair(remoteId, "unmuted");
          }
        }
        return;
      }
  ...
  useEffect(() => {
    applyUserMutedTrackChange(userMuted);

    if (muteDebounceTimerRef.current != null) {
      window.clearTimeout(muteDebounceTimerRef.current);
    }

    muteDebounceTimerRef.current = window.setTimeout(() => {
      muteDebounceTimerRef.current = null;
      applyDebouncedUserMutedPeerAttach(userMuted);
    }, MUTE_TOGGLE_DEBOUNCE_MS);
    ...
    applyDebouncedUserMutedPeerAttach,
    applyUserMutedTrackChange,
    userMuted,
  ]);
  ...
  useEffect(() => {
    ...
    const localExitedIds = new Set<string>();
    for (const member of members) {
      const id = String(member.device_id ?? "").trim();
      if (!id || id === deviceId) continue;
      if (hasLocalLeftCall(sessionId, id)) localExitedIds.add(id);
    }
    const memberSummary = formatVoiceBootstrapMemberSummary(members, {
      selfDeviceId: deviceId,
      remoteIds,
      localExitedIds,
      explicitRemovedIds: remotePeerGraceRefsRef.current.explicitRemoved,
    });
    const localTrackLive = isLocalTrackLive(
      localAudioTrackRef,
      localStreamRef
    ...
    const canOfferNow = canSendVoiceOffer();
    voiceProdLogOnStateChange(
      `voice-bootstrap:offer-effect:${sessionId}:${deviceId}`,
      [
        signalReady ? 1 : 0,
        micReady ? 1 : 0,
        remoteIds.length,
        remoteIds.map((id) => id.slice(-4)).join(","),
        memberSummary,
        canOfferNow ? 1 : 0,
        localTrackState,
        localCallEntryGenerationRef.current,
      ].join("|"),
      `[voice-bootstrap] offer-effect session=${sessionId.slice(-6)} ` +
        `device=${compactDeviceId(deviceId)} ` +
        `generation=${localCallEntryGenerationRef.current} ` +
        `signalReady=${signalReady ? 1 : 0} micReady=${micReady ? 1 : 0} ` +
        `canSendVoiceOffer=${canOfferNow ? 1 : 0} localTrack=${localTrackState} ` +
        `remoteIds=${remoteIds.length}` +
        `[${remoteIds.map((id) => compactDeviceId(id)).join(",") || "-"}] ` +
    ...
    if (!signalReady) {
      ...
      voiceProdLogOnStateChange(
        `voice-bootstrap:signal-not-ready:${sessionId}:${deviceId}`,
        `0|${remoteIds.length}`,
        `[voice-bootstrap] gate=signalReady_false session=${sessionId.slice(-6)} ` +
          `device=${compactDeviceId(deviceId)} remoteIds=${remoteIds.length}` +
          `[${remoteIds.map((id) => compactDeviceId(id)).join(",") || "-"}]`
      );
    ...
    if (remoteIds.length < 1) {
      const strict = getStrictRemoteIds();
      voiceProdLogOnStateChange(
        `voice-bootstrap:no-remoteIds:${sessionId}:${deviceId}`,
        `0|${strict.length}|${memberSummary}`,
        `[voice-bootstrap] gate=remoteIds_empty session=${sessionId.slice(-6)} ` +
          `device=${compactDeviceId(deviceId)} strictCount=${strict.length} ` +
          `members=${memberSummary || "-"}`
      );
      debugConsoleLog(
    ...
    const epochChanges = detectRemoteVoiceEpochChanges(
      members,
      deviceId,
      remoteVoiceEpochTracksRef.current
    );
    ...
    for (const change of epochChanges) {
      ...
      rejoiningRemoteIds.add(change.remoteId);
      void resetRemotePeerForRejoin(
        change.remoteId,
        "remote_rejoin",
        change.newEpoch
      );
    ...
    if (localCallEntryGenerationRef.current > 1) {
      ...
      for (const remoteId of remoteIds) {
        ...
        rejoiningRemoteIds.add(remoteId);
        const oldConnectionId = getCurrentConnectionId(remoteId);
        voiceProdLog(
          `[voice-bootstrap] peer-state-reset reason=local_rejoin ` +
            `remote=${compactDeviceId(remoteId)} ` +
            `voiceEpoch=${localCallEntryGenerationRef.current} ` +
            `connectionId=${compactConnectionId(oldConnectionId)} ` +
            `role=${deviceId < remoteId ? "active" : "passive"}`
        );
        void resetRemotePeerForRejoin(
          remoteId,
    ...
    for (const remoteId of remoteIds) {
      if (rejoiningRemoteIds.has(remoteId)) {
        voiceProdLogOnStateChange(
          `voice-bootstrap:action:${remoteId}`,
          `skip|local_rejoin_in_flight|${localCallEntryGenerationRef.current}`,
          `[voice-bootstrap] action=skip remote=${compactDeviceId(remoteId)} ` +
            `skipReason=local_rejoin_in_flight ` +
            `isOfferOwner=${deviceId < remoteId ? 1 : 0} ` +
            `hasUsablePc=${isUsablePeerConnection(pcsRef.current.get(remoteId)) ? 1 : 0} ` +
            `canSendVoiceOffer=${canSendVoiceOffer() ? 1 : 0} ` +
            `localTrack=${getLocalTrackReadyState(localAudioTrackRef, localStreamRef)}`
        );
        continue;
      }

      if (!remoteJoinPhaseRef.current.has(remoteId)) {
        remoteJoinPhaseRef.current.set(remoteId, "initial_connect");
      }
      ...
      const isOfferOwner = deviceId < remoteId;
      const logBootstrapAction = (
        action: "ensure" | "offer" | "skip",
        skipReason: string
      ) => {
        voiceProdLogOnStateChange(
          `voice-bootstrap:action:${remoteId}`,
          `${action}|${skipReason}|${hasUsablePc ? 1 : 0}|${canSendVoiceOffer() ? 1 : 0}|${getLocalTrackReadyState(localAudioTrackRef, localStreamRef)}|${phase}`,
          `[voice-bootstrap] action=${action} remote=${compactDeviceId(remoteId)} ` +
            `skipReason=${skipReason} hasUsablePc=${hasUsablePc ? 1 : 0} ` +
            `isOfferOwner=${isOfferOwner ? 1 : 0} ` +
            `canSendVoiceOffer=${canSendVoiceOffer() ? 1 : 0} ` +
            `localTrack=${getLocalTrackReadyState(localAudioTrackRef, localStreamRef)} ` +
            `phase=${phase}`
        );
      ...
      if (!shouldEnsure) {
        if (establishedSkip || shouldSuppressPassiveOfferReschedule(phase)) {
          ...
          logBootstrapAction(
            "skip",
            establishedSkip
              ? `established_${establishedSkip}`
              : `phase_${phase}`
          );
          continue;
        }
        if (hasUsablePc) {
          logBootstrapAction("skip", "has_usable_pc");
          continue;
        }
      }
      ...
      if (
        ...
        passiveJoinSettledRef.current.add(remoteId);
        logBootstrapAction("skip", `phase_${phase}`);
        continue;
      }
      ...
      if (isOfferOwner) {
        if (!activeCanOffer) {
          let skipReason = "active_cannot_offer";
          if (!voiceSettingsReadyRef.current) {
            skipReason = "settings_not_ready";
            logVoiceStartBlocked(remoteId, "settings_not_ready");
          } else if (!signalReady) {
            skipReason = "signal_not_ready";
            logVoiceStartBlocked(remoteId, "signal_not_ready");
          } else if (
            relayForcedRef.current &&
            !hasTurnIceServer(iceServersRef.current)
          ) {
            skipReason = "turn_not_loaded";
            logVoiceStartBlocked(remoteId, "turn_not_loaded");
          } else if (!micReady) {
            skipReason = "mic_not_ready";
            logVoiceStartBlocked(remoteId, "mic_not_ready");
          } else if (!localTrackLive && !receiveOnly) {
            skipReason = "local_track_not_live";
            logVoiceStartBlocked(remoteId, "local_track_not_live");
          }
          logBootstrapAction("skip", skipReason);
          continue;
        }
        ...
        logBootstrapAction("offer", "maybe_start_offer");
        void maybeStartOffer(remoteId);
      } else {
        if (shouldSuppressPassiveOfferReschedule(phase)) {
          passiveJoinSettledRef.current.add(remoteId);
        ...
        if (phase === "idle_unnegotiated") {
          if (passiveCanWait) {
            logBootstrapAction("ensure", "passive_on_join_wait");
            beginPassiveOfferWait(remoteId, "passive_on_join", {
              initialJoin: true,
            });
          } else if (!voiceSettingsReadyRef.current) {
            logVoiceStartBlocked(remoteId, "settings_not_ready");
            logBootstrapAction("skip", "settings_not_ready");
          } else if (!signalReady) {
            logVoiceStartBlocked(remoteId, "signal_not_ready");
            logBootstrapAction("skip", "signal_not_ready");
          } else if (
            relayForcedRef.current &&
            !hasTurnIceServer(iceServersRef.current)
          ) {
            logVoiceStartBlocked(remoteId, "turn_not_loaded");
            logBootstrapAction("skip", "turn_not_loaded");
          } else {
            logBootstrapAction("skip", "passive_cannot_wait");
          }
          continue;
        }
        logBootstrapAction("ensure", "passive_on_join");
  ...
  useEffect(() => {
    ...
    const micBecameReady = micReady && !prevMicReadyForAttachRef.current;
    const streamBecameReady =
      localStreamReady && !prevLocalStreamReadyForAttachRef.current;
    const signalBecameReady =
      signalReady && !prevSignalReadyForAttachRef.current;

    prevMicReadyForAttachRef.current = micReady;
    prevLocalStreamReadyForAttachRef.current = localStreamReady;
    prevSignalReadyForAttachRef.current = signalReady;

    if (!micBecameReady && !streamBecameReady && !signalBecameReady) {
      return;
    }
    ...
    const reason = streamBecameReady
      ? micBecameReady
        ? "mic_and_stream_ready"
        : "local_stream_ready"
      : signalBecameReady
        ? "signal_ready"
        : "mic_ready";
