-- Separate voice vs chat match pools per theme (sessions.entry_mode).
ALTER TABLE public.sessions
  ADD COLUMN IF NOT EXISTS entry_mode text NOT NULL DEFAULT 'voice';

ALTER TABLE public.sessions DROP CONSTRAINT IF EXISTS sessions_entry_mode_check;
ALTER TABLE public.sessions
  ADD CONSTRAINT sessions_entry_mode_check CHECK (entry_mode IN ('voice', 'chat'));

CREATE INDEX IF NOT EXISTS sessions_class_entry_mode_status_idx
  ON public.sessions (class_id, entry_mode, status, created_at);

-- Exclude locked / join-window-closed sessions from match-join candidate search.
-- Late join and leaver refill must not select members_locked_at or expired join_open_until.
-- Open forming sessions remain merge targets. Signature/grants unchanged (idempotent REPLACE).
-- Post-join lock guards and capacity checks are preserved.

-- Predicate used by candidate filters and schema verify probes.
CREATE OR REPLACE FUNCTION public.session_open_for_match_join(p_session_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $$
  SELECT EXISTS (
    ...
    WHERE s.id = p_session_id
      AND s.members_locked_at IS NULL
      AND (s.join_open_until IS NULL OR s.join_open_until >= now())
  );
$$;

GRANT EXECUTE ON FUNCTION public.session_open_for_match_join(uuid)
  TO service_role;
...
CREATE OR REPLACE FUNCTION public.match_join_atomic_v3(
  ...
  p_entry_mode text DEFAULT 'voice'
)
RETURNS TABLE (
  class_id uuid,
  class_name text,
...
BEGIN
  ...
  v_entry_mode := CASE
    WHEN lower(btrim(COALESCE(p_entry_mode, 'voice'))) = 'chat' THEN 'chat'
    ELSE 'voice'
  END;
  ...
  ELSE
    IF NOT v_recruitment_unlimited THEN
      ...
      WHERE c.id = s.class_id::uuid
        ...
        AND COALESCE(s.entry_mode, 'voice') = v_entry_mode;
    ...
    v_match_lock_key :=
      'match:' ||
      v_world_key || ':' ||
      COALESCE(p_topic_key, 'free') || ':' || v_entry_mode;
    ...
    IF NOT v_found_class THEN
      ...
      INSERT INTO public.sessions (class_id, topic, status, capacity, entry_mode)
      VALUES (v_class_id::text, v_class_name, 'forming', v_requested_capacity, v_entry_mode)
      ...
      WHERE c2.world_key = v_world_key
        ...
        AND s2.id IS DISTINCT FROM v_chosen_session_id
          AND COALESCE(s2.entry_mode, 'voice') = v_entry_mode
      ORDER BY
        (
          SELECT count(*)::integer
  ...
  WHERE (
    ...
    AND public.is_billable_class_for_slots(c.name, c.lifecycle);
            ...
            AND (
              ...
              OR EXISTS (
                ...
                WHERE sm.session_id = s.id
                  AND (
                    sm.device_id = p_device_id
                    OR (v_user_id IS NOT NULL AND sm.user_id = v_user_id)
                  )
              )
            )
          )
  ...
  -- Reject join after members are locked (post open-window / capacity).
  IF EXISTS (
    SELECT 1
    FROM public.sessions s
    WHERE s.id = v_chosen_session_id
      AND s.members_locked_at IS NOT NULL
  ) AND NOT EXISTS (
    SELECT 1
    FROM public.session_members sm
    WHERE sm.session_id = v_chosen_session_id
      AND (
        sm.device_id = p_device_id
        OR (v_user_id IS NOT NULL AND sm.user_id = v_user_id)
      )
  ) THEN
    RAISE EXCEPTION 'session_members_locked'
      USING DETAIL = json_build_object('sessionId', v_chosen_session_id)::text;
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE s.id = v_chosen_session_id
      AND s.members_locked_at IS NULL
      AND s.join_open_until IS NOT NULL
      AND s.join_open_until < now()
  ) AND NOT EXISTS (
    SELECT 1
    FROM public.session_members sm
    WHERE sm.session_id = v_chosen_session_id
    ...
    UPDATE public.sessions
    SET members_locked_at = COALESCE(members_locked_at, now())
    WHERE id = v_chosen_session_id;
    RAISE EXCEPTION 'session_members_locked'
      USING DETAIL = json_build_object('sessionId', v_chosen_session_id)::text;
  END IF;
  ...
  SELECT COALESCE(s.capacity, v_requested_capacity)
  INTO v_capacity
  FROM public.sessions s
  WHERE s.id = v_chosen_session_id;
  ...
  IF v_member_count >= 3 THEN
    UPDATE public.sessions s
    SET join_open_until = COALESCE(s.join_open_until, now() + interval '30 seconds')
    WHERE s.id = v_chosen_session_id
      AND s.members_locked_at IS NULL;
  END IF;
  ...
  IF v_member_count >= v_capacity THEN
    UPDATE public.sessions
    SET
      members_locked_at = COALESCE(members_locked_at, now()),
      join_open_until = COALESCE(join_open_until, now())
    WHERE id = v_chosen_session_id;
  ELSIF EXISTS (
    SELECT 1
    FROM public.sessions s
...
GRANT EXECUTE ON FUNCTION public.match_join_atomic_v3(
  text, text, uuid, text, text, integer, integer, text[], integer, integer, text
) TO service_role;
