-- Normal match: reuse only classes with fresh forming/waiting sessions.
-- If none joinable, create a NEW classes row + forming session (never attach
-- a new forming session to a stale/active-only class).
-- Forced/openJoinedClass path unchanged.
