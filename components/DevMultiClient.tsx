...
type DevPlanPreset = "free" | "basic" | "standard" | "premium";
...
function buildEntitlement(
  ...
  return {
    plan: classSlots >= 5 ? "slots_5" : classSlots >= 3 ? "slots_3" : "free",
    class_slots: classSlots,
    topic_plan: 0,
    can_create_classes: classSlots > 1,
    theme_pass: false,
...
async function pushEntitlementOverrideToServer(
  deviceId: string,
  value: DevEntitlementOverride
) {
  const r = await fetch("/api/dev/set-entitlements", {
    method: "POST",
  ...
  if (!r.ok || !json?.ok) {
    throw new Error(json?.error || "set_entitlements_failed");
  }
        ...
        {users.map((user) => (
          <button
            ...
            style={{
              textAlign: "left",
              border:
                user.devKey === selectedDevKey
                  ? "2px solid #111827"
                  : "1px solid #e5e7eb",
              borderRadius: 12,
              background: user.devKey === selectedDevKey ? "#f9fafb" : "#fff",
              padding: 12,
              cursor: "pointer",
            }}
          >
  ...
  return (
    ...
    >
      {users.map((user) => {
        ...
        const srcWithRefresh =
          refreshTick > 0
            ? `${frameUrl}${frameUrl.includes("?") ? "&" : "?"}refresh=${refreshTick}`
            : frameUrl;
  ...
  async function syncAllEntitlements() {
    ...
    try {
      for (const user of users) {
        ...
        const ent = buildEntitlement(user.planPreset, user.classSlots);

        try {
          const result = await pushEntitlementOverrideToServer(deviceId, ent);
          console.log("[dev-console] entitlement synced =", {
            devKey: user.devKey,
            deviceId,
            result,
          });
        } catch (e) {
          console.error("[dev-console] entitlement sync failed", {
            devKey: user.devKey,
            deviceId,
            error: e,
          });
    ...
    >
      ...
      >
        <DevSidebar
          ...
          syncAllEntitlements={syncAllEntitlements}
          syncing={syncing}
        />
